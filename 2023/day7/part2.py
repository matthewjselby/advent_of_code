...
def identify_hand(hand_info):
    card_map = hand_info['map'].copy()
    if 'J' in card_map and len(card_map) > 1:
        joker_count = card_map['J']
        card_map.pop('J')
        most_frequent_card = max(card_map, key=card_map.get)
        card_map[most_frequent_card] += joker_count
    card_counts = sorted(card_map.values(), reverse=True)
    if 5 in card_counts:
